...
export const EmailVerificationTemplate = ({ code }: EmailVerificationTemplateProps) => {
    return (
        <Html>
            <Head />
            <Preview>
                Verify your email address to complete your {env.APP_TITLE} registration
            </Preview>
            <Body style={main}>
                <Container style={container}>
                    <Section>
                        <Text style={title}>{env.APP_TITLE}</Text>
                        <Text style={text}>Hi,</Text>
                        <Text style={text}>
                            Thank you for registering for an account on {env.APP_TITLE}. To complete
                            your registration, please verify your your account by using the
                            following code:
                        </Text>
                        <Link href={`${env.NEXT_PUBLIC_APP_URL}/verify-email?code=${code}`}>
